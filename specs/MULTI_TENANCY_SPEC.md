...
**Status (2026-08-21): Slices 1–3 implemented (application code); Slice 4 (infra) not started.**
...
- **Slice 4 (infra deployment) — not started, deliberately.** Needs a real `SUBDOMAIN_HOST` domain decision (explicitly a placeholder per this spec's own scope note) and production VPS/Caddy/DNS changes — out of an agent's unilateral judgment call per this repo's risk-confirmation norms. Application code above is dormant everywhere `CLOUD` is unset, so it carries zero risk to Gameloops until Slice 4 actually stands up a second deployment.

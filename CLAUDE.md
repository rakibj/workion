...
`kanban` page type. Backend: `core/kanban/`, tables `kanban_columns`/`kanban_cards`/`kanban_card_assignees`/`kanban_milestones`/`kanban_categories`/`kanban_category_options`/`kanban_card_category_values`. Frontend: `features/kanban/`, Atlaskit pragmatic DnD; assignees, priority, and page-level custom **Categories** (a category has a name + a preset icon and owns a list of colored options; a card picks at most one option per category — same single-select interaction model as Milestone, just user-definable per board). Priority/Milestone/Category badges on the card face render icon-only when unset and a colored pill with the label once a value is set. Realtime: WS events `kanbanCardMoved`/`kanbanColumnMoved` broadcast on the `space-${spaceId}` room (not a per-page room), filtered by `userId` to skip self. Milestone badge turns red (overdue) / amber (today).

**Per-card due date** — `kanban_cards.due_date` (nullable `date` column, see `docs/specs/done/KANBAN_CARD_DUE_DATE_SPEC.md`), independent of Milestone: a card can have a Milestone, a due date, both, or neither. Set/cleared via `UpdateCardDto.dueDate` on the existing card-update endpoint (same nullable pattern as `milestoneId`), and via the `update_kanban_card` AI chat tool (`dueDate: 'none'` to clear). Card-face badge (`IconCalendarDue`, distinct from Milestone's `IconTarget`) opens a `DateInput` popover rather than a menu, since it's a free date value, not a pick from a shared list; reuses the same `getDueDateStatus`/`DUE_DATE_COLOR` red/amber thresholds as Milestone.

**Spec (source of truth):** `docs/specs/ongoing/KANBAN_CATEGORIES_AND_AI_TOOLS_SPEC.md` — Spec 1 (icon-only badges), Spec 2 (custom Categories), and Spec 3 (AI chat board actions — create/move/update card + set category via the chat panel's tool-calling) are all implemented. The file stays in `ongoing/` because Spec 3 is unit-tested only — no live in-browser smoke test with a real OpenRouter key has been run yet.

When the chat panel's `contextPageId` is a kanban page the user can edit, `AiChatController` registers four AI SDK tools (`create_kanban_card`, `move_kanban_card`, `update_kanban_card`, `set_kanban_card_category`) built from real column/card/milestone/category ids that `formatKanbanAsText` now prints inline in the board context text. Each tool verifies its target belongs to the bound page and returns `{ok:false, error}` rather than throwing on a bad id; a successful mutation broadcasts a page-scoped `invalidate` WS event so the open board updates without a manual refresh. `AiStreamService.streamChat` takes an optional `tools` param and sets `stopWhen: stepCountIs(5)` for multi-step tool calling.
...
- [docs/specs/ongoing/KANBAN_CATEGORIES_AND_AI_TOOLS_SPEC.md](docs/specs/ongoing/KANBAN_CATEGORIES_AND_AI_TOOLS_SPEC.md) — Kanban icon-only badges (Spec 1), custom Categories (Spec 2), and AI chat panel tool-calling to create/move/update Kanban cards and set categories (Spec 3) are all implemented (2026-08-31). File stays in `ongoing/` pending Spec 3's live in-browser smoke test with a real OpenRouter key.

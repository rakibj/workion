...
export interface ToggleHeadingContentOptions {
  HTMLAttributes: Record<string, any>;
}

export const ToggleHeadingContent = Node.create<ToggleHeadingContentOptions>({
  name: "toggleHeadingContent",
  group: "block",
  content: "block*",
  defining: true,
  selectable: false,
  ...
  addKeyboardShortcuts() {
    return {
      // Exit the toggle when pressing Enter on the last empty child
      Enter: ({ editor }) => {
        const { view, state } = editor;
        const { selection } = state;

        const findNode = findParentNode(
        ...
        if (
          !(childCount === selection.$from.index(findNode.depth) + 1)
        ) {
          return false;
        }
        ...
        if (!lastNode.eq(fillNode)) return false;

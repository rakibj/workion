...
| 6   | Browser smoke testing + production domain verification | In progress | 2026-07-29 | Full stack deployed to production (`workion.gameloops.io`) and the entire public API surface (list, detail, primary-domain SSR, sitemap/rss/robots via selector, SPA-fallback regression) verified against a throwaway DB-fixture post, then cleaned up — see below. Still open: the documented browser smoke-test procedure (steps 1–7) using the real authenticated UI, and a real custom-domain + DNS/Caddy check. |
...
**2026-07-29 production deploy note:** the first deploy of this batch crash-looped — `BlogPublicService` injects `TokenService` (Spec 7), but `BlogModule` only imported `ShareModule`, which imports `TokenModule` without re-exporting it, so it wasn't visible to `BlogModule`. Fixed by adding `TokenModule` directly to `BlogModule`'s `imports` (commit `d5ea1c5`). Caught immediately because the container kept restarting and `docker compose logs app` showed a clear `UnknownDependenciesException`. Also see the `baseUrl` percent-encoding gotcha noted under Spec 9's Definition of Done — same root cause (`ignoreDuplicateSlashes`) would bite any query param carrying a raw URL.
